...
describe('tester les fonctionnalités du client', () => {
  ...
  it('Vérifier le tableau de bord et cliquer sur modifier le profil', function() {
    cy.visit('http://127.0.0.1:8000/comptes/dashboard/');
    cy.contains('Voir les menus').should('be.visible');
    cy.contains('Voir mes commande').should('be.visible');
    cy.contains('Modifier').should('be.visible');
    cy.get('#modifprofil').click();
    cy.url().should('include', 'http://127.0.0.1:8000/comptes/profile/');
  });

  it('Vérifier la page de modif du profil', function() {
    cy.visit('http://127.0.0.1:8000/comptes/profile/');
    cy.get('input[name=username]').clear();
    cy.get('input[name=username]').type('Michelle');
  });

  it('Vérifier la page des menus', function() {
    cy.visit('http://127.0.0.1:8000/menus/');
    cy.url().should('include', '/menus/');
    cy.contains('Spécialités Nos Menus').should('be.visible');
  });

  it('Vérifier que toutes les catégories sont présentes sur la page menus', function() {
    cy.visit('http://127.0.0.1:8000/menus/');
    cy.contains('Entrées').should('be.visible');
    cy.contains('Plats de résistance').should('be.visible');
    cy.contains('Desserts').should('be.visible');
  ...
  it('Tester le bouton voir détails et contenu', function() {
    cy.visit('http://127.0.0.1:8000/menus/');
    cy.get('button[id="id_detailsbuton"]').first().click();
    cy.get('.modal.show').should('exist'); // vérifier que la modal apparaît
    cy.get('.modal.show .modal-title').should('not.be.empty'); // vérifier le contenu (le titre)
  });

  it('Tester les fonctionnalités du panier vide', function() {
    cy.visit('http://127.0.0.1:8000/commandes/panier/');
    cy.contains('Votre panier est vide.');
    cy.contains('Ajouter un menu').should('be.visible');
  });
  ...
  describe('panier non vide', () => {
    ...
    it('Le panier non vide doit s\'afficher', function() {
      cy.visit('http://127.0.0.1:8000/commandes/panier/');
      cy.contains('Mon Panier');
      cy.contains('Total');
      cy.contains('Prix unitaire');
    ...
    it('Doit pouvoir augmenter la quantité du menu', function() {
      cy.visit('http://127.0.0.1:8000/commandes/panier/');
      cy.contains('Mon Panier');
      cy.get('#AugQMenu').dblclick();
    });

    it('Doit pouvoir diminuer la quantité du menu', function() {
      cy.visit('http://127.0.0.1:8000/commandes/panier/');
      cy.contains('Mon Panier');
      cy.get('#dimQMenu').click();
    });
    ...
    it('Tester le bouton passer la commande', function() {
      cy.visit('http://127.0.0.1:8000/commandes/panier/');
      cy.get('.btn.btn-success').click();
      cy.url().should('include', 'http://127.0.0.1:8000/commandes/passer-commande/');
    });

    it('Tester le bouton continuer mes achats', function() {
      cy.visit('http://127.0.0.1:8000/commandes/panier/');
      cy.get('.btn.btn-primary').click();
      cy.url().should('include', 'http://127.0.0.1:8000/menus/');
    });

    it('Passer la commande', function() {
      cy.visit('http://127.0.0.1:8000/commandes/passer-commande/');
      cy.contains('Confirmer la commande').should('be.visible');
      cy.get('.fa.fa-check').click(); // clique sur le bouton valider la commande
      cy.url().should('include', 'http://127.0.0.1:8000/commandes/mes-commandes/');
    });
  });
